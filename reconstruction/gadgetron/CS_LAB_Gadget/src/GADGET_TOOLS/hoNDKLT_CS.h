...
namespace Gadgetron
{
	...
	*/
	template <typename T> class EXPORTCSLAB hoNDKLT_CS
	{
	public:
		typedef typename realType<T>::Type value_type;
		typedef hoNDKLT_CS<T> Self;

		hoNDKLT_CS();
		hoNDKLT_CS(const hoNDArray<T> &data, size_t dim, size_t output_length);
		hoNDKLT_CS(const hoNDArray<T> &data, size_t dim, value_type thres);
		hoNDKLT_CS(const Self &v);
		virtual ~hoNDKLT_CS();
		...
		// MS - backward/forward transform
		void ftransform(const hoNDArray<T> &in, hoNDArray<T> &out, size_t dim = 0) const;
		void btransform(const hoNDArray<T> &in, hoNDArray<T> &out, size_t dim = 0) const;
