...
namespace Gadgetron
{
	...
	*/
	...
	public:
		...
		// MS - with scrambling
		void fft(hoNDArray<ComplexType> *input, unsigned int dim_to_transform, bool bScrambling)
		{
			// -1 refers to the sign of the transform, -1 for FFTW_FORWARD
			fft_int(input,dim_to_transform,-1, bScrambling);
		}

		void ifft(hoNDArray<ComplexType> *input, unsigned int dim_to_transform, bool bScrambling)
		{
			// 1 refers to the sign of the transform, +1 for FFTW_BACKWARD
			fft_int(input,dim_to_transform,1, bScrambling);
		}

		void fft(hoNDArray<ComplexType> *input, bool bScrambling)
		{
			for (size_t i = 0; i < input->get_number_of_dimensions(); i++) {
				// -1 refers to the sign of the transform, -1 for FFTW_FORWARD
				fft_int(input,i,-1, bScrambling);
			}
		}

		void ifft(hoNDArray<ComplexType> *input, bool bScrambling)
		{
			for (size_t i = 0; i < input->get_number_of_dimensions(); i++) {
				// 1 refers to the sign of the transform, +1 for FFTW_BACKWARD
				fft_int(input,i,1,bScrambling);
