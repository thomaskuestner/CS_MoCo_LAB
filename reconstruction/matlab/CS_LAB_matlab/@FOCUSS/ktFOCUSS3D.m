function dImg = ktFOCUSS3D(obj, kSpace)
% 3D FOCUSS algorithm
%
% input:
...
for iI = 1:obj.iNOUTER % outer loop for kt-FOCUSS
    ...
    for iJ = 1:obj.iNINNER % inner loop for conjugate gradient
        e = kSpace - obj.fullMask.*bTrafo(rho); % ^= v - F*rho; kSpace (k_y-k_z-x-cha); rho (y-z-x-cha new_base); e (k_y-k_z-x-cha (cart))
...
%         G = -conj(W).*fTrafo(e) .* nPha .* nZ + obj.lambda .*q + obj.lambdaCalib .* kernelMult(obj.lambdaCalib) + .5* obj.lambdaTV .* gradTV(obj.lambdaTV) + obj.lambdaESPReSSo .* gradESPReSSO(obj.lambdaESPReSSo);
        G = -conj(W).*fTrafo(e) + obj.lambda .*q + obj.lambdaCalib .* kernelMult(obj.lambdaCalib) + .5* obj.lambdaTV .* gradTV(obj.lambdaTV) + obj.lambdaESPReSSo .* gradESPReSSO(obj.lambdaESPReSSo);
%         beta = zeros(nPha,nZ,nFreq,nCha,obj.measPara.precision);
%         if(isa(G,'TRAFO'))
...
%         alpha = zeros(nPha,nZ,nFreq,nCha,obj.measPara.precision); 
%         if(isa(d,'TRAFO'))
...
%         displayInterRes( kernelBTrafo(rho) );
        ...
        if((obj.espresso.state && obj.espresso.reconType == 2) || (~obj.espresso.state && obj.espresso.reconType == 2 && obj.lambdaESPReSSo > 0))
...
%             mask = obj.fullMask(:,:,:,1); % y-z-x
    ...
    %         end
            ...
            if(obj.espresso.norm == 1) % L1
                l1Smooth = 1e-15;
                rhoTmp = bTrafo(conj(W.*q).*phase + W.*q);
                if(any(obj.trafo.fftBA(1,:))) % go back to kSpace
                    for iBA=find(obj.trafo.fftBA(1,:))
                        rhoTmp = fftnshift(rhoTmp,iBA);
                    end
                ...
                Gesp = Gesp(:)'*conj(Gesp(:));
                out = (Gesp+l1Smooth).^(-0.5) .* (2 .* conj(q) + conj(W) .* W .* q .* conj(phase) - conj(W) .* fTrafo(kSpaceCombi) .* conj(phase));
...
%                 out = 0.5 * conj(W) .* (q .* (1+phase) + W .* conj(q) .* phase - fTrafo(kSpaceCombi) .* (1+phase)); 
                out = 2 .* conj(q) + conj(W) .* W .* q .* conj(phase) - conj(W) .* fTrafo(kSpaceCombi) .* conj(phase);
            end
        elseif(obj.espresso.constraint == 2) % Im = 0
            g = W.*q - abs(W.*q);
            if(obj.espresso.norm == 1) % L1
                A = 0.5 .* conj(g)./(abs(g) + eps);
                B = W - 0.5 .* conj(W.*q)./(abs(W.*q) + eps);
                C = conj(A);
                D = -0.5 .* conj(W.*q)./(abs(W.*q) + eps);
                out = conj(A.*B + C.*D);
            else % L2
                f = sqrt(g(:)'*g(:));
                A = conj(g)./(2*f + eps);
