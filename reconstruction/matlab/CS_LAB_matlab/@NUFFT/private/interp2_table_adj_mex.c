...
const mxArray *mx_K)
{
	...
	if (!mxIsInt32n(mx_J, 2))
		mexErrMsgTxt("J must be int32 [1,2]");
	if (!mxIsInt32n(mx_L, 2))
		mexErrMsgTxt("L must be int32 [1,2]");
	if (!mxIsInt32n(mx_K, 2))
		mexErrMsgTxt("K must be int32 [1,2]");
	...
	if (M != mxGetM(mx_tm) || 2 != mxGetN(mx_tm))
		mexErrMsgTxt("t_m must be Mx2 matrix");
	...
	for (nn=0; nn < N; ++nn) {
		interp2_table_adj(r_ck, i_ck, Kd[0], Kd[1],
			r_h1, i_h1, r_h2, i_h2,
...
const int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs != 7)
		mexErrMsgTxt("7 inputs needed: (f, h1, h2, J, L, t, K)");
	if (nlhs > 1)
	...
	if (!interp2_table_adj_mex(plhs,
		...
		prhs[5], prhs[6]))
		mexErrMsgTxt("interp2_table_adj_mex() failed");
