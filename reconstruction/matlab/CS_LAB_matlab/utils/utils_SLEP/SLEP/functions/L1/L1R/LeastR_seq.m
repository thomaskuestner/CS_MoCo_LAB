function [x, funVal, ValueL]=LeastR_seq(z, opts)
%
%%
% Function LeastR
...
if (opts.nFlag==0)
    ...
    % replaced with the equivalent one (March 27, 2010)
    flag=2;
    ATy=matrixVectorFile(opts, flag, y);
elseif (opts.nFlag==1)
    %ATy=A'*y;
    % replaced with the equivalent one (March 27, 2010)
    flag=2;
    ...
    % replaced with the equivalent one (March 27, 2010)
    flag=2;
    ATy=matrixVectorFile(opts, flag, invNu);
    
    ATy=ATy-sum(invNu)*mu';
end
...
if (opts.mFlag==0 && opts.lFlag==0)
    ...
    for iterStep=1:opts.maxIter
        ...
        if (opts.nFlag==0)
            ...
            % replaced with the equivalent one (March 27, 2010)
            flag=2;
            ATAs=matrixVectorFile(opts, flag, As);
        elseif (opts.nFlag==1)
            %ATAs=A'*As;
            % replaced with the equivalent one (March 27, 2010)
            flag=2;
            ...
            ATAs=ATAs- sum(As) * mu';  ATAs=ATAs./nu;
        else
            invNu=As./nu;                
            ...
            % replaced with the equivalent one (March 27, 2010)
            flag=2;
            ATAs=matrixVectorFile(opts, flag, invNu);
            
            ATAs=ATAs-sum(invNu)*mu';
        end
...
if(opts.mFlag==1 && opts.lFlag==1)
    ...
    if (opts.nFlag==0)
        ...
        % replaced with the equivalent one (March 27, 2010)
        flag=2;
        ATAx=matrixVectorFile(opts, flag, Ax);
    elseif (opts.nFlag==1)
        %ATAx=A'*Ax;
        % replaced with the equivalent one (March 27, 2010)
        flag=2;
        ...
        ATAx=ATAx- sum(Ax) * mu';  ATAx=ATAx./nu;
    else
        invNu=Ax./nu;
        ...
        % replaced with the equivalent one (March 27, 2010)
        flag=2;
        ATAx=matrixVectorFile(opts, flag, invNu);
        
        ATAx=ATAx-sum(invNu)*mu';
    end
