...
% 	fprintf('minmax:kb	max%%diff = %g', max_percent_diff(Y.d,Y.mmkb))
	...
	Y.kb = nufft(x, s.kb);
	fprintf('kaiser		max%%diff = %g', max_percent_diff(Y.d,Y.kb))
	...
	Y.ku = nufft(x, s.ku);
	fprintf('kaiser-user	max%%diff = %g', max_percent_diff(Y.d,Y.ku))
	...
	Y.mm = nufft(x, s.mm);
	fprintf('minmax:user	max%%diff = %g', max_percent_diff(Y.d,Y.mm))

	if 0	% test 'uniform' scaling
		s.un = nufft_init(om, Nd, Jd, Kd, n_shift, 'uniform')
		Y.un = nufft(x, s.un);
		fprintf('user-unif max%%diff = %g', max_percent_diff(Y.mm,Y.un))
	return
	end
...
%	printf('KB 2d vs nd max%%diff = %g', max_percent_diff(Y2,Y.kb))
	fprintf('KB 2d vs nd max%%diff = %g', max_percent_diff(s2.p,s.kb.p))

	s2 = nufft2_init(om, oldarg{:}, n_shift, 0, 'best');
	fprintf('tuned 2d vs nd max%%diff = %g', max_percent_diff(s2.p,s.mmtu.p))

	s2 = nufft2_init(om, oldarg{:}, n_shift, 0); % minmax
	fprintf('user 2d vs nd max%%diff = %g', max_percent_diff(s2.p,s.mm.p))
