#include <mex.h>
#include <matrix.h>
// rearrange column vector to 3D array (complex-valued, single precission)
//
// (c) Thomas Kuestner
// ---------------------------------------------------------------------
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
  ...
  float *indataReal = (float*)mxGetPr(I);
  //double *indataImag;
  //if( mxIsComplex(I) )
  float *indataImag = (float*)mxGetPi(I);
  ...
  //double *outdataReal;
  //double *outdataImag;
  mxArray *out = mxCreateNumericArray( 2, dims, mxSINGLE_CLASS, mxCOMPLEX );
  float *outdataReal = (float*)mxGetPr(out);
  float *outdataImag = (float*)mxGetPi(out);
  /*if ( !mxIsComplex(I) ) {
