function [ x_NLTV ] = SB_NLTVfunc_slim_rescale( x,n1,n2,lambdaNLTV_SB, lambdaNLTV_h )
...
% opts.h0=15; %% weight filter parameter, depends on noise and image standard variation. for example: for barbara [0, 255], h0=20; To be adapted for normalized image
  x_max = max(max(abs(x)));
  x = abs(x)./x_max .* 255;
  %lambdaNLTV_SB = lambdaNLTV_SB; % good value ~1
  %lambdaNLTV_h = lambdaNLTV_h; % good value ~10
...
opts.nDenoising=10; %% 3st level: denoising/regularization level, in general could be fixed to be 10
