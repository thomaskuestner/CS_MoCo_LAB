...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    /* Check if all values are inputs are of type float*/
    for(i=0; i<nrhs; i++) {
        if(!mxIsSingle(prhs[i])) { mexErrMsgTxt("Inputs must be float"); }
    }
    ...
    /* Connect vectors */
    V=(float *)mxGetData(prhs[1]);
    
    /* Set Values */
    T=(float *)mxGetData(prhs[2]);
    kernelratio=(int)T[0];
    
    /* Set Values */
    T=(float *)mxGetData(prhs[3]);
    windowratio=(int)T[0];
    ...
    Nthreadsd=(float *)mxGetData(prhs[5]);
    ...
    plhs[0] = mxCreateNumericArray(ndimsJ, dimsJ, mxSINGLE_CLASS, mxREAL);
    J=(float *)mxGetData(plhs[0]);
